...
export async function getPageStaticPropsForPath(params, preview = false) {
  ...
  const seoData = await client.item(itemSystemInfo.codename)
    .elementsParameter(["seo__title", "seo__description", "seo__keywords", "seo__canonical_url", "seo__options"])
    .toPromise()
    .then(response => ({
      title: response.item["seo__title"].value,
      description: response.item["seo__description"].value,
      keywords: response.item["seo__keywords"].value,
      canonicalUrl: response.item["seo__canonical_url"].value,
      noIndex: response.item["seo__options"].value.some(item => item.codename == "no_index"),
    }));
